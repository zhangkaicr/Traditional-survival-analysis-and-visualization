...
data_final$new_os <- (data_final$OS-data_final$rfs)
attach(data_final)
#以下进行亚组限定

n<- which(rfs<=16&recurrence=="no")
data_final <- data_final[-n,]
#以下绘制生存曲线
data_final$sur_time <- data_final$new_os
data_final$sur_status <- data_final$status
data_final$sur_status<- ifelse(data_final$sur_status=="death",1,0 )

fit <- survfit(Surv(sur_time,sur_status) ~ team16,  # 创建生存对象 
